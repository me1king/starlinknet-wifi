...
export default function AdminDashboard() {
    ...
    try {
      ...
      const safeFetch = async (url: string) => {
        try {
          ...
          if (!res.ok) {
            console.error(`[Dashboard] Fetch failed for ${url}: ${res.status} ${res.statusText}`);
            return null;
          }
          const text = await res.text();
          if (text.includes('<!DOCTYPE')) {
             console.warn(`[Dashboard] HTML response detected for ${url} (Likely a redirect or tunnel page)`);
             return null;
          }
          try {
            return JSON.parse(text);
          } catch (parseErr) {
            console.error(`[Dashboard] JSON Parse Error for ${url}:`, text.substring(0, 100));
            return null;
          }
        } catch (e) {
          console.error(`[Dashboard] Fetch exception for ${url}:`, e);
          return null;
        }
      };
  ...
  const handleResendVoucherByRef = async (ref: string, phone: string) => {
    ...
    try {
        ...
        alert("❌ Network error while attempting to resend.");
    }
  };
              ...
              <div className="lg:col-span-1 space-y-6">
                ...
                <div className="h-24 flex items-end gap-1 px-1">
                    {latencyLogs.slice().reverse().map((log, i) => {
                        ...
                        return (
                            <div key={log.id} className="flex-1 bg-indigo-500/20 hover:bg-indigo-500/40 border-t-2 border-indigo-500 rounded-t-sm transition-all relative group" style={{ height: `${height}%` }}>
                    ...
                    <div className="flex justify-between items-center mb-8">
                        <div className="flex items-center gap-6">
                            <button
                                ...
                                className={`text-sm font-black uppercase tracking-widest flex items-center gap-2 ${!showHistory ? 'text-white border-b-2 border-indigo-500 pb-1' : 'text-gray-500 hover:text-white'}`}
                            >
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-gray-950/50 text-[9px] text-gray-600 uppercase font-black border-b border-gray-800">
                                <tr>
                                    ...
                                    <th className="p-4 text-right tracking-widest">{showHistory ? 'Time / Duration' : 'Connected'}</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-800/30">
